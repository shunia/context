...
//			}
		...
		private static function initContext(context:Object):Object {
			...
			var item:IContextPlugin = null;
			if (specContext is Class) {
				item = new specContext() as IContextPlugin;
			} else {
				item = specContext as IContextPlugin;
			}
			if (item) {
				item.startUp(_stage, context.hasOwnProperty("param") && context["param"] ? context["param"] : variables);
			}
